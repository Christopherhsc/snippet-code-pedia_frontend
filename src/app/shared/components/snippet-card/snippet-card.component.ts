...
export class SnippetCardComponent implements OnInit, OnChanges {
  ...
  balanceSnippets() {
    ...
    const snippetHeights = this.snippets.map((snippet) =>
      this.estimateSnippetHeight(snippet)
    )
    ...
    if (this.balancedColumns[0].length === 0 && this.balancedColumns[1].length > 0) {
      this.balancedColumns[0] = this.balancedColumns[1].splice(
        0,
        Math.ceil(this.balancedColumns[1].length / 2)
      )
    } else if (
      this.balancedColumns[1].length === 0 &&
      this.balancedColumns[0].length > 0
    ) {
      this.balancedColumns[1] = this.balancedColumns[0].splice(
        0,
        Math.ceil(this.balancedColumns[0].length / 2)
      )
    }
  }
  ...
  estimateImageHeight(picture: any): number {
    const aspectRatio =
      picture.width && picture.height ? picture.height / picture.width : 1
